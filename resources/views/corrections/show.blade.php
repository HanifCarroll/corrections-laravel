        ...
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900 dark:text-gray-100">
                ...
                @foreach($post->sentences as $postSentence)
                    ...
                    @endphp
                    <div
                        class="mb-6 p-4 border rounded {{ $isCorrected ? 'bg-yellow-50 dark:bg-yellow-900 border-yellow-300 dark:border-yellow-600' : 'bg-green-50 dark:bg-green-900 border-green-300 dark:border-green-600' }}">
                        <div class="mb-2">
                            <label
                                class="block text-sm font-medium {{ $isCorrected ? 'text-yellow-700 dark:text-yellow-300' : 'text-green-700 dark:text-green-300' }}">
                        ...
                        @if($isCorrected)
                            <div class="mb-2">
                                <label
                                    class="block text-sm font-medium text-yellow-700 dark:text-yellow-300">Correction</label>
                ...
                <div class="mt-6 flex space-x-8">
                    <x-secondary-button href="{{ route('posts.show', $correction->post) }}">
                        Back to Post
                    </x-secondary-button>
                    @if($correction->user_id === auth()->id())
                        <form action="{{ route('posts.corrections.destroy', [$correction->post, $correction]) }}"
                              method="POST"
                              onsubmit="return confirm('Are you sure you want to delete this correction?');">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
